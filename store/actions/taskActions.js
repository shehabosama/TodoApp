...
export const getTasks=(onSuccess=()=>{} , onError=()=>{})=>{
    return async dispatch=>{
        try{
            const taskRes = await AsyncStorage.getItem(STORAGE_KEYS.tasks);
            const tasks = taskRes ? JSON.parse(taskRes):[];
...
export const updateTask =(task , onSuccess=()=>{} , onError=()=>{})=>{
    return async dispatch=>{
        try{
            const {tasks} = store.getState().task;
            const updateTask = [...tasks].map(t=> t.id === task.id? task : t);
            await AsyncStorage.setItem(STORAGE_KEYS.tasks , JSON.stringify(updateTask));
...
export const deleteTask =(id , onSuccess=()=>{} , onError=()=>{})=>{
    return async dispatch=>{
        try{
            ...
            await AsyncStorage.setItem(STORAGE_KEYS.tasks , JSON.stringify(updateTasks));
